...
void save_file(){
        int i,j,col=grid_w,line=grid_h;
        FILE *fp;
...
void update_matrix(int thread_id){
    int i,j;
    //cout<<first<<endl;
    int decomp_data_size=grid_h/decomp_h;
    int inter_start=thread_id*decomp_data_size,inter_end=thread_id==decomp_h-1?grid_h:(thread_id+1)*decomp_data_size;
    //cout<<thread_id<<" "<<inter_start<<" "<<inter_end<<endl;
    for(i=inter_start;i<inter_end;i++){
        for(j=0;j<grid_h;j++){
            int q,neig=0;
            for(q=0;q<8;q++){
...
{ int thread_id;
  ...
  int steps=2000;
   ...
   while(i++<steps){
    ...
    cout<<endl;
    display_grid(false);
    ofstream MyFile("measure1D.log");
    MyFile<<grid_h<<" "<<grid_w<<endl;
    MyFile<<decomp_h<<endl;
    float sum=0;
    for(i=0;i<steps;i++){
